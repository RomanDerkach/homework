...
func BenchmarkFilterBooks(b *testing.B) {
	...
	filter, err := json.Marshal(storage.BookFilter{Title: "title"})
	if err != nil {
		b.Fatal("cant marshal filter")
	}
	bookIO := bytes.NewReader(filter)
	req := httptest.NewRequest("POST", "/books/helpmewithbooks", bookIO)
	...
	for i := 0; i < b.N; i++ {
		...
		handler.booksHandlerByIDPOST(rw, req)
